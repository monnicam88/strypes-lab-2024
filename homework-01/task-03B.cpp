...
public:
    ...
    float calcVolumeInCubicMeters() const {
        ...
        const float areaOfTriangle = radiusMinusHeight * std::sqrt(2 * barrelRadius * barrelFilledHeight - heightSquared);
    ...
    float getFilledHeight() const { 
        return barrelFilledHeight; 
    }
};
...
int main() {
    ...
    if (std::cin.fail()) {
        std::cerr << "Error: Program failed to read barrel length" << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    std::cout << "Volume of horizontal cylinder is: " << calc.calcVolumeInCubicMeters() << " m^3" << std::endl;
