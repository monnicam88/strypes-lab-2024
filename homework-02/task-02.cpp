...
// The Kaham summation algorithm - used to improve the accuracy of adding a series of
...
// Plain floating-point summation
template <typename FloatT, typename = std::enable_if_t<std::is_floating_point_v<FloatT>>>
...
void runTestCase(const std::string& message, const FloatT f1, const FloatT f2,
    ...
    if (compFunc(f1, f2)) 
        std::cout << "[PASSED]\n";
    else std::cout << "[FAILED WITH DIFF] " << std::fabs(f1 - f2) << "\n";
}

template <typename FloatT, typename = std::enable_if_t<std::is_floating_point_v<FloatT>>>
void runTests(const FloatT randRealNum) {
    static int32_t testCounter = 1;
    std::cout << "============= Run comparison test #" << testCounter++ << " =============" << std::endl;
    ...
    std::cout << "Kaham sum number: " << std::fixed << std::setprecision(sizeof(randRealNum) * 8)
        << kahamSumValue << std::endl;
    std::cout << "Plain sum number: " << std::fixed << std::setprecision(sizeof(randRealNum) * 8)
        << plainSumValue << std::endl;

    std::cout << ">>> KAHAM SUM TESTS:" << std::endl;
    ...
    std::cout << ">>> PLAIN SUM TESTS:" << std::endl;
...
int main() {
    ...
    for (size_t i = 1; i < (size_t)1e12; i *= 2) {
        // Switch line 110 & line 111 to test for float and double
